...
cd /home/ubuntu/cp-jmgoode/gas
source /home/ubuntu/cp-jmgoode/gas/.env
[[ -d /home/ubuntu/cp-jmgoode/gas/log ]] || mkdir /home/ubuntu/cp-jmgoode/gas/log
if [ ! -e /home/ubuntu/cp-jmgoode/gas/log/$GAS_LOG_FILE_NAME ]; then
    touch /home/ubuntu/cp-jmgoode/gas/log/$GAS_LOG_FILE_NAME;
fi
if [ "$1" = "console" ]; then
    LOG_TARGET=-
else
    LOG_TARGET=/home/ubuntu/cp-jmgoode/gas/log/$GAS_LOG_FILE_NAME
fi
/home/ubuntu/.virtualenvs/mpcs/bin/gunicorn --log-file=$LOG_TARGET \
--log-level=debug --workers=$GUNICORN_WORKERS --certfile=/usr/local/src/ssl/ucmpcs.org.crt \
--keyfile=/usr/local/src/ssl/ucmpcs.org.key --bind=$GAS_APP_HOST:$GAS_HOST_PORT gas:app
